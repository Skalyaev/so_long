...
int check_map_walls(t_map *map_infos, int size, int len, int i)
{
        ...
        while (++i >= 0 && map_infos->map_struct[i] && map_infos->map_struct[i] != '\n' && ++len)
        ...
        while (map_infos->map_struct[++i] && ++size)
        {
                if (map_infos->map_struct[i] != '1')
                        return (0);
                j = 0;
                while (map_infos->map_struct[i + j] && map_infos->map_struct[i + j] != '\n')
                        j++;
                if (j != len)
                        return (0);
                i += j;
                if (!map_infos->map_struct[i] && !check_last_wall(map_infos, size, len, &i))
                        return (0);
        }
        return (1);
}
...
int check_map_infos(t_map *map_infos, int p, int e, int c)
{
        ...
        {
                if (map_infos->map_struct[i] != '0' && map_infos->map_struct[i] != '1' && map_infos->map_struct[i] != 'C' && map_infos->map_struct[i] != 'E' && map_infos->map_struct[i] != 'P' && map_infos->map_struct[i] != '\n')
