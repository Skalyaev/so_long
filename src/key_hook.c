...
{
    if (keycode == 115)
        if (mlx->x < mlx->map_infos->map_size && mlx->map_infos->map_tab[mlx->x + 1][mlx->y] != 1 && ++mlx->moves && ++mlx->redraw)
            mlx->x += 1;
    if (keycode == 113)
        if (mlx->y > 0 && mlx->map_infos->map_tab[mlx->x][mlx->y - 1] != 1 && ++mlx->moves && ++mlx->redraw)
            mlx->y -= 1;
    if (keycode == 100)
        if (mlx->y < mlx->map_infos->map_len - 1 && mlx->map_infos->map_tab[mlx->x][mlx->y + 1] != 1 && ++mlx->moves && ++mlx->redraw)
            mlx->y += 1;
    if (mlx->y < mlx->old_y)
        mlx->dir = 1;
...
int keyrelease_hook(int keycode, t_mlx *mlx)
{
    ...
    if (keycode == 122)
        if (mlx->x > 0 && mlx->map_infos->map_tab[mlx->x - 1][mlx->y] != 1 && ++mlx->moves && ++mlx->redraw)
            mlx->x -= 1;
